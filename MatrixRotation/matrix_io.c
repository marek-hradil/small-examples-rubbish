...
inline static void _delete_matrix(Matrix *matrix)
{
...
inline static void _fill_matrix_from_file(FILE *f, Matrix *matrix)
{
    for (int i = 0; i < matrix->rows; i++)
    {
...
inline static Matrix *_load_matrix_from_file(FILE *f)
{
...
Matrix *load_matrix_from_file(char *path)
{
    ...
    if (f == NULL)
    {
        perror("Couldn't open input matrix file.");
        exit(EXIT_FAILURE);
    }

    Matrix *matrix = _load_matrix_from_file(f);
    if (fclose(f) != 0)
    {
        perror("Couldn't close input matrix file.");
        exit(EXIT_FAILURE);
    }
...
inline static void _print_matrix_to_file(FILE *f, Matrix *matrix)
{
...
void print_matrix_to_file(char *path, Matrix *matrix)
{
    ...
    if (f == NULL)
    {
        perror("Couldn't open output matrix file.");
        exit(EXIT_FAILURE);
    }
    ...
    if (fclose(f) != 0)
    {
        perror("Couldn't close output matrix file.");
        exit(EXIT_FAILURE);
    }
}
