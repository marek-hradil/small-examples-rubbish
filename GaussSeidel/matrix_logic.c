...
inline static void _gauss_seidel(Matrix *matrix, float accuarcy, int iteration_limit)
{
...
void gauss_seidel(Matrix *matrix, float accuarcy, int iteration_limit)
{
    assert(matrix != NULL && matrix->cols > matrix->rows);
    assert(accuarcy > 0);
    ...
    _gauss_seidel(matrix, accuarcy, iteration_limit);
}
